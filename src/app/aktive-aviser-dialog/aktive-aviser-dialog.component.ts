...
export class AktiveAviserDialogComponent implements OnInit {
  ...
  dataSoure = new MatTableDataSource<AktiveAviser>();
  ...
  constructor(private ak: AktiveaviserService, private dialog: MatDialog, private obs: StamBladObserver) {
   this.loadData();
...
private loadData() {
  ...
  ngOnInit() {
    ...
    this.Tlf1.valueChanges.subscribe(value =>  {
      this.filterValues.Tlf1 = value;
      this.dataSoure.filter = JSON.stringify(this.filterValues);
    });
  ...
  createFilter(): (data: any, filter: string) => boolean {
    const filterFunction = function(data, filters): boolean {
      ...
      return data.PostBy1.toLowerCase().indexOf(searchTerms.PostBy1) !== -1
        && data.PostNr1.toString().toLowerCase().indexOf(searchTerms.PostNr1) !== -1
        && data.Navn1.toLowerCase().indexOf(searchTerms.Navn1) !== -1
      ...
      && data.Tlf1.toString().toLowerCase().indexOf(searchTerms.Tlf1) !==  -1
  ...
  public openDialog() {
    const promise  = this.dialog.open(AktiveaviserColumnDialogComponent, { width: '10vh', height: '80%', autoFocus: true, ariaLabel: 'label'}).afterClosed().toPromise()
    promise.then(value => console.log(value.text));
  }

  applyFilter(filterValue: any) {
    this.dataSoure.filter = filterValue.target.value.trim().toLowerCase();
  }
